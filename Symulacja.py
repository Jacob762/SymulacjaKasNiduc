...
class Simulation:
    ...
    def simulatione(self, dniPracy: int, godzinyPracy):
        self.gui.kasa_change_color(0,"green")
            ...
            for godziny in range(godzinyPracy):
                ...
                for i in range(60):
                    ...
                    while len(self.klientela) > 0 and j < len(self.lista):
                        a = self.lista[j].dodajKlienta(self.klientela.pop(0))
                        if a == 7:
                            j += 1
                            time.sleep(0.1)
                            self.gui.klienci_change_color(j, len(self.lista[j].getKlienci()))
                            time.sleep(0.1)
                    ...
                    for k in range(len(self.lista)):
                        ...
                        if self.lista[k].getActive(): print(self.lista[k].getIlePusta(), " KASA NR ", k,
                                                            " STOI TYLE RUND PUSTA")
